import { useSession } from "next-auth/react";
import Image from "next/image";
import { Message } from "../typings";
import TimeAgo from "react-timeago";

type Props = {
  message: Message;
};

function MessageComponent({ message }: Props) {
  const { data: session } = useSession();
  const isUser = session?.user?.email === message.email;

  return (
    <>
      {isUser ? (
        <div className="flex w-fit ml-auto">
          <div className="flex-shrink-0 order-2">
            <Image
              className="rounded-full mx-2"
              height={10}
              width={40}
              src={message.profilePic}
              alt="Profile Picture"
              style={{ height: "auto", width: "auto" }}
              priority
            />
          </div>

          <div>
            <p className="text-[0.65rem] px-[2px] pb-[2px] text-blue-400 text-right">
              {message.username}
            </p>

            <div className="flex items-end">
              <div className="px-3 py-2 rounded-lg w-fit text-white bg-blue-400 ml-auto order-2">
                <p>{message.message}</p>
              </div>

              <p className="text-[0.65rem] italic px-2 text-gray-300 text-right">
                <TimeAgo date={new Date(message.created_at)} />
              </p>
            </div>
          </div>
        </div>
      ) : (
        <div className={`flex w-fit`}>
          <div className={`flex-shrink-0`}>
            <Image
              className="rounded-full mx-2"
              height={10}
              width={40}
              src={message.profilePic}
              alt="Profile Picture"
              style={{ height: "auto", width: "auto" }}
              priority
            />
          </div>

          <div>
            <p
              className={`text-[0.65rem] px-[2px] pb-[2px] text-red-400 text-left`}
            >
              {message.username}
            </p>

            <div className="flex items-end">
              <div
                className={`px-3 py-2 rounded-lg w-fit text-white bg-red-400`}
              >
                <p>{message.message}</p>
              </div>

              <p className={`text-[0.65rem] italic px-2 text-gray-300`}>
                <TimeAgo date={new Date(message.created_at)} />
              </p>
            </div>
          </div>
        </div>
      )}
    </>
  );
}

export default MessageComponent;
